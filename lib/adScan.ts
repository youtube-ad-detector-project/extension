...
export type VideoRisk = {
  ...
  riskSource: 'blended_sentence_risk' | 'none';
...
//   한 문장 최대값만으로 영상 전체를 대표하면 고위험 룰 1건에도 점수가 과하게 포화되므로,
//   최고 문장·상위 3문장 평균·반복 신호를 섞어 "주의 신호 지수"로 완만하게 집계한다.
export function calculateVideoRisk(lines: ScannedLine[]): VideoRisk {
  ...
  const positiveRisks = risks.filter((risk) => risk > 0).sort((a, b) => b - a);
  const sMax = positiveRisks[0] ?? 0;
  const suspiciousSentenceCount = positiveRisks.length;
  ...
  const topThreeAverage =
    ((positiveRisks[0] ?? 0) +
      ...
      (positiveRisks[2] ?? 0)) /
    3;
  const frequencySignal = 1 - Math.exp(-suspiciousSentenceCount / 3);
  const countBonus = Math.round(frequencySignal * 13);
  const rawRiskScore = sMax * 65 + topThreeAverage * 20 + frequencySignal * 13;
  const riskScore = Math.min(98, Math.round(rawRiskScore));
  ...
  if (riskScore >= 70) {
    riskGrade = '높음';
    riskLevelText = '주의 신호 높음';
  } else if (riskScore >= 40) {
    riskGrade = '중간';
    riskLevelText = '일부 주의 신호 감지';
  } else {
    riskGrade = '낮음';
    riskLevelText = '약한 주의 신호 감지';
  }
  ...
  return {
    ...
    riskGrade,
    riskLevelText,
    riskSource: 'blended_sentence_risk',
    suspiciousSentenceCount,
    maxSentenceRiskPercent: Math.round(sMax * 100),
    countBonus,
    calculationNote:
      '주의 신호 지수는 실제 위법 확률이 아니라 최고 문장 신호, 상위 문장 묶음, 반복 신호를 종합한 자동 탐지 기준 점수입니다.',
  };
}
